...
usage() {
  cat <<'EOF'
Usage:
  run_localisation_campaign_detached.sh start [run_localisation_campaign args...]
  run_localisation_campaign_detached.sh status <screen_name>
  run_localisation_campaign_detached.sh stop <screen_name>
EOF
}
...
start_job() {
  ...
  local screen_name="${SCREEN_NAME:-localisation_campaign_${batch_id}}"
  local batch_root="${BATCH_ROOT:-${BASE_RUN_ROOT}/${batch_id}}"
  local launcher_log="${LAUNCH_LOG:-${batch_root}/launcher.log}"
  local launch_command_path="${batch_root}/launch_command.txt"
  ...
  {
    printf 'cd %q\n' "${ROOT_DIR}"
    printf 'BATCH_ID=%q BATCH_ROOT=%q /bin/bash ./run_localisation_campaign.sh %s\n' "${batch_id}" "${batch_root}" "${quoted_args}"
  } > "${launch_command_path}"

  cmd="cd ${quoted_root} && export BATCH_ID=${quoted_batch_id} BATCH_ROOT=${quoted_batch_root} && exec /bin/bash ./run_localisation_campaign.sh ${quoted_args} >> ${quoted_launcher_log} 2>&1"
