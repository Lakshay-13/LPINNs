...
usage() {
  cat <<'EOF'
Usage:
  run_gaussian_validation_detached.sh start [run_gaussian_validation args...]
  run_gaussian_validation_detached.sh status <screen_name>
  run_gaussian_validation_detached.sh stop <screen_name>
EOF
}
...
start_job() {
  ...
  local screen_name="${SCREEN_NAME:-localisation_gaussian_${batch_id}}"
  ...
  cmd="cd ${quoted_root} && export BATCH_ID=${quoted_batch_id} BATCH_ROOT=${quoted_batch_root} && exec /bin/bash ./run_gaussian_validation.sh ${quoted_args} >> ${quoted_launcher_log} 2>&1"
