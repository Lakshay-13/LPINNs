...
class CustomNN(nn.Module):
    def __init__(self, n_input_units=2, hidden_units=[16,16], actv=SineActivation, n_output_units=1, t_min=0, t_max=1,
...
class HE():
    def __init__(self, input_neurons=[32,32], lr = 1e-2, activation = nn.Tanh, sigma = True, initial_sigma=None, localization=True):
        ...
        if localization:
            nets = [CustomNN(n_input_units=2, n_output_units=1, hidden_units=input_neurons, actv=activation, sigma_learnable=sigma, initial_sigma=initial_sigma)]
        else:
