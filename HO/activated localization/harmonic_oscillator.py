...
def render(solver, name=None):
    ...
    if flag:
        axs[0][1].plot(solver.best_nets[0].mu.cpu().detach().numpy(), solver.best_nets[0].sigma.cpu().detach().numpy(), marker='o')
...
class CustomNN(nn.Module):
    def __init__(self, n_input_units=1, hidden_units=[16,16], actv=SineActivation, n_output_units=1, t_min=0, t_max=1, mu_learnable=False, sigma_learnable=False):
