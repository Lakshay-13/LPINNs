...
def render(solver, name=None):
    ...
    if flag:
        axs[0][1].plot(mus, sigmas, color=colors[1], alpha = 0.6)
    ...
    t_vals = torch.linspace(solver.t_min, solver.t_max/4, 256, requires_grad = True)
    residuals = solver.get_residuals(t_vals)
    derivatives = []
    if flag:
        for res in residuals:
            derivatives.append(diff(res**2, solver.best_nets[0].layers[0].weight,shape_check=False))
    else:
        for res in residuals:
...
class CustomNN(nn.Module):
    def __init__(self, n_input_units=1, hidden_units=[16,16], actv=nn.Tanh, n_output_units=1, t_min=0, t_max=1, 
                 mu_learnable=False, sigma_learnable=True, initial_sigma=None, sigma_min=None, sigma_max=None, amplitude = 1.0):
        ...
        if not initial_sigma:
            if not sigma_min:
                initial_sigma = (t_max-t_min)/hidden_units[0]
                if sigma_learnable:
                    self.sigma = nn.Parameter(torch.ones(hidden_units[0])*initial_sigma)
                else:
                    self.sigma = torch.ones(hidden_units[0])*(initial_sigma)
            else:
                if sigma_learnable:
                    self.sigma = torch.randint(-1,2,(hidden_units[0],))*(torch.rand(hidden_units[0])*(sigma_max-sigma_min) + sigma_min)
                    self.sigma[self.sigma==0.0] = 0.001
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i == 1:
                ...
                layer_output[~mask] = 0
                layer_output[mask] = layer_output[mask]*self.amplitude

        # Output layer transformation
        output = self.fc_out(layer_output)
        return output
...
class HO():
    def __init__(self, t_0=0.0, u_0=0.0, u_0_prime = 1.0, t_min=0.0, t_max=4*np.pi, 
                 ...
                 input_neurons=[32,32], lr = 1e-2, activation = nn.SiLU, initial_sigma=None, sigma_min=None, sigma_max=None, 
                 localization=True, scaling = True, amplitude = 1.0):
        ...
        if localization:
            nets = [CustomNN(n_input_units=1, n_output_units=1, hidden_units=input_neurons, actv=activation, t_min=t_min, t_max=t_max, 
                            mu_learnable=mu, sigma_learnable=sigma, initial_sigma=initial_sigma, sigma_min=sigma_min, sigma_max=sigma_max, amplitude = amplitude)]
        else:
            nets = [FCNN(n_input_units=1, n_output_units=1, hidden_units=input_neurons, actv=activation)]
        if scaling:
