...
class CustomNN(nn.Module):
    def __init__(self, n_input_units=1, hidden_units=[16,16], actv=nn.Tanh, n_output_units=1, t_min=0, t_max=1, 
                 mu_learnable=False, sigma_learnable=True, initial_sigma=None, sigma_min=None, sigma_max=None):
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i == 1:
                layer_output = layer_output * torch.exp(- (x - self.mu) ** 20 / (2 * self.sigma ** 20))
...
class HO():
    def __init__(self, t_0=0.0, u_0=0.0, u_0_prime = 1.0, t_min=0.0, t_max=4*np.pi, 
                 ...
                 localization=True, scaling = True):
        ...
        if localization:
            nets = [CustomNN(n_input_units=1, n_output_units=1, hidden_units=input_neurons, actv=activation, t_min=t_min, t_max=t_max, 
                            mu_learnable=mu, sigma_learnable=sigma, initial_sigma=initial_sigma, sigma_min=sigma_min, sigma_max=sigma_max)]
        else:
