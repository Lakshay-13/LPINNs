...
def render(solver, name=None):
    ...
    try:
        ...
        temp = np.linspace(solver.t_min, solver.t_max, sigmas.shape[0])
        flag=True
    ...
    if flag:
        axs[0][1].plot(temp, sigmas, color=colors[1], alpha = 0.6)
...
class CustomNN(nn.Module):
    def __init__(self, n_input_units=1, hidden_units=[16,16], actv=nn.Tanh, n_output_units=1, t_min=0, t_max=1, 
                sigma_learnable=True, initial_sigma=None, sigma_min=None, sigma_max=None):
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i == 1:
                layer_output = layer_output * ((2/((3*self.sigma)**1/2*torch.pi**1/4))*(1-(x/self.sigma)**2)) * torch.exp(-(x**2/(2*self.sigma**2)))
...
class HO():
    def __init__(self, t_0=0.0, u_0=0.0, u_0_prime = 1.0, t_min=0.0, t_max=4*np.pi, 
                 train_batch_size=256, valid_batch_size=64, sigma=False,input_neurons=[32,32], lr = 1e-2, activation = nn.SiLU, 
                 initial_sigma=None, sigma_min=None, sigma_max=None, localization=True, scaling = True):
        ...
        if localization:
            nets = [CustomNN(n_input_units=1, n_output_units=1, hidden_units=input_neurons, actv=activation, t_min=t_min, t_max=t_max, 
                            sigma_learnable=sigma, initial_sigma=initial_sigma, sigma_min=sigma_min, sigma_max=sigma_max)]
        else:
