...
class CustomNN(nn.Module):
    def __init__(self, n_input_units=1, hidden_units=[16,16], actv=nn.Tanh, n_output_units=1, t_min=0, t_max=1, mu_learnable=False, sigma_learnable=True, initial_sigma=None):
...
class HO():
    def __init__(self, t_0=0.0, u_0=0.0, u_0_prime = 1.0, t_min=0.0, t_max=4*np.pi, 
                 ...
                 input_neurons=[32,32], lr = 1e-2, activation = nn.SiLU, initial_sigma=None):
        ...
        mu = mu
        sigma = sigma
        nets = [CustomNN(n_input_units=1, n_output_units=1, hidden_units=input_neurons, actv=activation, t_min=t_min, t_max=t_max, mu_learnable=mu, sigma_learnable=sigma, initial_sigma=initial_sigma)]
