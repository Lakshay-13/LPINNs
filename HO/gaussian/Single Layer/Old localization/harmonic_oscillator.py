    ...
    def __init__(self, n_input_units=1, hidden_units=[16,16], actv=nn.Tanh, n_output_units=1, t_min=0, t_max=1, 
        ...
        if not initial_sigma:
            if not sigma_min:
                ...
                if sigma_learnable:
                    # print('Sigmas are learnable')
                    self.sigma = nn.Parameter(torch.randint(-1,2,(hidden_units[0],))*(torch.rand(hidden_units[0])*(sigma_max-sigma_min) + sigma_min))
                else:
                    # print('Sigmas are not learnable')
