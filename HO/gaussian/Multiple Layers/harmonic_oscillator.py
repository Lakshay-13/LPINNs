...
class CustomNN(nn.Module):
    def __init__(self, n_input_units=1, hidden_units=[16,16], actv=SineActivation, n_output_units=1, t_min=0, t_max=1,
        ...
        for i in range(len(hidden_units)):
            ...
            if sigma_learnable:
                self.sigmas.append(nn.Parameter(torch.ones(hidden_units[i])*(initial_sigma)*hidden_units[0]/hidden_units[i]))
            else:
                self.sigmas.append(torch.ones(hidden_units[i])*(initial_sigma)*hidden_units[0]/hidden_units[i])
    ...
    def forward(self, x):
        ...
        for i in range(len(self.layers)-1):
            ...
            out = self.act(out)
            print(out.shape)
            print(self.mus[i].shape)
            out = out * torch.exp(- (x - self.mus[i]) ** 2 / (2 * self.sigmas[i] ** 2))
